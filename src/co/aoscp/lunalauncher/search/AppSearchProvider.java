package co.aoscp.lunalauncher.search;
...
public class AppSearchProvider extends ContentProvider
{
    ...
    public static Uri dm(final AppInfo appInfo, final UserManagerCompat userManagerCompat) {
        return new Uri.Builder().scheme("content").authority("co.aoscp.lunalauncher.appssearch").appendQueryParameter("component", appInfo.componentName.flattenToShortString()).appendQueryParameter("user", Long.toString(userManagerCompat.getSerialNumberForUser(appInfo.user))).build();
    }
    ...
    private Cursor dn(final List list) {
        ...
        while (iterator.hasNext()) {
            ...
            final int n2 = n + 1;
            row.add(n).add(appInfo.title.toString()).add(string).add("co.aoscp.lunalauncher.search.APP_LAUNCH").add(string);
            n = n2;
        }
        return matrixCursor;
    }
