...
public class LunaIconProvider extends IconProvider implements Runnable {
    ...
    public void run() {
        ...
        if (IconPackUtils.isPackProvider(mContext, mIconPack)) {
            try {
    ...
    private boolean isCalendar(String packageName) {
        return GOOGLE_CALENDAR.equals(packageName);
    }
    ...
    public Drawable getIcon(LauncherActivityInfo launcherActivityInfo, int iconDpi, boolean flattenDrawable) {
        ...
        if (drawable == null) {
            ...
            if ((!Utilities.ATLEAST_OREO || !(drawable instanceof AdaptiveIconDrawable)) &&
                    !GOOGLE_CALENDAR.equals(packageName)) {
    ...
    public String getIconSystemState(final String packageName) {
        return isCalendar(packageName) ? mSystemState + " " + getDayOfMonth() : mSystemState;
    }
}